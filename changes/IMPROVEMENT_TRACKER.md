...
## Verification Log
- **Syntax Verification**: `python -m py_compile backend/rag_graph.py` $\rightarrow$ Passed (Exit Code 0).
- **Retrieval Test**: Verified that queries like `"what is a turbine?"` activate query expansion, hybrid BM25+FAISS RRF search, and soft fallback retrieval, preventing empty candidate lists.
